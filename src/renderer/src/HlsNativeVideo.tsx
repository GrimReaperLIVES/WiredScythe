...
export function HlsNativeVideo({ state, target = "main" }: HlsNativeVideoProps) {
  ...
  useEffect(() => {
    ...
    // hls.js's bandwidthEstimate measures downloads from WiredScythe's local
    ...
    const enableStabilityProfile = () => {
      ...
      // at WiredScythe's normal low-latency target.
    ...
    const onPlay = () => {
      ...
      // instead of going through WiredScythe's go-live command. Treat that as a
    ...
    const onPause = () => {
      if (document.pictureInPictureElement === video && playbackRequested) {
        ...
        // command handler. The custom WiredScythe PiP controls use the normal
