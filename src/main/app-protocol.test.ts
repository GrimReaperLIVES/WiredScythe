...
describe("app protocol", () => {
  ...
  beforeEach(async () => {
    rendererDirectory = await mkdtemp(
      path.join(tmpdir(), "wiredscythe-app-protocol-"),
    );
    await mkdir(path.join(rendererDirectory, "assets"));
    await writeFile(
      path.join(rendererDirectory, "index.html"),
      "<main>WiredScythe</main>",
    );
    fetchFile = vi.fn(
      async () =>
        new Response("<main>WiredScythe</main>", {
  ...
  it("serves emote artwork from the app's own store", async () => {
    const response = await handler(
      new Request(
        `wiredscythe://app/emote?src=${encodeURIComponent("https://cdn.7tv.app/emote/kept/1x.webp")}`,
      ),
    );
  ...
  it("does not serve an emote address the store refused", async () => {
    const response = await handler(
      new Request(
        `wiredscythe://app/emote?src=${encodeURIComponent("https://example.test/evil.webp")}`,
      ),
    );
  ...
  it("does not treat a bare emote request as a file to read", async () => {
    const response = await handler(new Request("wiredscythe://app/emote"));
  ...
  it("serves the root index with hardened no-cache headers", async () => {
    const response = await handler(new Request("wiredscythe://app/"));

    expect(response.status).toBe(200);
    expect(response.headers.get("cache-control")).toBe("no-cache");
    expect(response.headers.get("x-content-type-options")).toBe("nosniff");
    expect(await response.text()).toBe("<main>WiredScythe</main>");
    expect(fetchFile).toHaveBeenCalledOnce();
  });

  it("rejects non-read requests before touching the filesystem", async () => {
    const response = await handler(
      new Request("wiredscythe://app/index.html", { method: "POST" }),
    );
  ...
  it("rejects another authority and directories", async () => {
    const foreign = await handler(new Request("wiredscythe://other/index.html"));
    const directory = await handler(new Request("wiredscythe://app/assets/"));
  ...
  it("maps encoded assets inside the renderer directory", () => {
    ...
    ).toBe(path.resolve(rendererDirectory, "assets", "app shell.js"));
  });
