...
// client applications. WiredScythe's public Device Code client never uses a secret.
...
export class TwitchService {
  ...
  async initialize(): Promise<void> {
    // Remove WiredScythe's retired user-configurable Client ID file. Client IDs are
    // public application identifiers and WiredScythe now ships its registered one.
  ...
  async updateChatColor(rawColor: TwitchChatColorInput): Promise<TwitchChatColorState> {
    ...
    if (!this.token?.scopes.includes("user:manage:chat_color")) {
      throw new Error(
        "Sign in with Twitch again to grant WiredScythe permission to change your chat color.",
      );
    }
