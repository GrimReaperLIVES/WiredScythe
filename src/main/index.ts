...
// WiredScythe keeps its own isolated application data, credentials, and
// website sessions rather than inheriting another application's profile.
app.setPath("userData", path.join(app.getPath("appData"), "WiredScythe"));
...
const applicationIcon = app.isPackaged
  ...
  : path.join(currentDirectory, "../../build/wiredscythe-icon.png");
...
const KICK_WEBSITE_PARTITION = "persist:wiredscythe-kick";
...
async function openKickWindow(slug: string, title: string): Promise<void> {
  ...
  const window = new BrowserWindow({
    ...
    webPreferences: {
      ...
      partition: "persist:wiredscythe-kick",
...
const SUBSCRIPTION_CLOSE_URL = "https://wiredscythe.invalid/close";
...
handleTrusted(
  ...
  // The embedded Standard player is unreliable without a signed-in browser
  ...
  if (effectiveMode === "native") {
    ...
    if (openGeneration !== playerOpenGeneration || activeChannelName !== channel) {
      ...
      return { channel, mode: effectiveMode };
    }
    if (!nativeResult.ok) fallbackReason = nativeResult.reason;
  }

  activePlayerMode = effectiveMode;
  latestNativePlayerState =
    effectiveMode === "native" ? hlsNativePlayer.getState() : null;
  updateThumbnailToolbar();
  ...
  return {
    ...
    fallbackReason:
      ...
      (requestedMode === "official"
        ? "Standard playback fell back to WiredScythe Native so the public stream can play."
        : undefined),
  };
  },
);
...
app.whenReady().then(async () => {
  app.setAppUserModelId("app.wiredscythe.viewer");
  // WiredScythe provides its own application UI. Removing Electron's default
