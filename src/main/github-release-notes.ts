...
export class GitHubReleaseNotesService {
  ...
  constructor(
    ...
    private readonly releasesUrl: string | undefined = RELEASES_URL,
  ) {}
  ...
  private async fetchMarkdown(): Promise<string> {
    if (!this.releasesUrl) {
      throw new Error("No WiredScythe release-notes feed is configured.");
    }
    const response = await this.fetcher(this.releasesUrl, {
      headers: {
        Accept: "application/vnd.github+json",
        "User-Agent": `WiredScythe/${app.getVersion()}`,
