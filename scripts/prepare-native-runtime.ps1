...
# Copy instead of moving from the expanded archive. On Windows, Move-Item can
# leave a package directory locked mid-transfer, which produces a partial
# runtime (and an installer that starts but cannot play streams).
Get-ChildItem -LiteralPath $streamlinkRoot.FullName -Force |
  Copy-Item -Destination $streamlinkDirectory -Recurse -Force
try {
  Remove-WorkspaceDirectory $streamlinkStaging
} catch {
  # The staging folder is safe to leave behind if a virus scanner briefly
  # holds a handle. It is excluded from the packaged runtime and replaced on
  # the next preparation run.
  Write-Warning "Could not remove temporary Streamlink staging files."
}
